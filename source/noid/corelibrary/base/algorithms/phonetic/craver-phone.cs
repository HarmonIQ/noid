...
namespace NoID.Base.Algorithms
{
    internal static class CaverPhone
    {
        ...
        static Regex _alpha = new Regex("[^a-z]", RegexOptions.Compiled);
        static Regex _lower_vowel = new Regex("[aeiou]", RegexOptions.Compiled);
        ...
        public static bool GenerateCaverphone(string name, out string key)
        {
            ...
            key = name.ToLower();
            key = _alpha.Replace(key, string.Empty);
        ...
        private static string TranslateRemaining(string key)
        {
            ...
            if (_lower_vowel.IsMatch(key.Substring(0, 1)))
            {
                key = "A" + key.Substring(1 < nameLength ? 1 : nameLength);
            }            
            //all other vowels with a 3
            key = _lower_vowel.Replace(key, "3");
