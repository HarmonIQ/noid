...
namespace SourceAFIS.Simple
{
    ...
    public class AfisEngine
    {
        ...
        public void ExtractFingerprint(Fingerprint fp)
        {
            lock (this)
            {
                TemplateBuilder builder = Extractor.Extract(fp.Image, Dpi);
        ...
        List<Template> FlattenHierarchyFinger(Fingerprint[] fingerprints, Finger finger, out List<int> fingerIdexes)
        {
            List<Template> templates = new List<Template>();
            fingerIdexes = new List<int>();
            for (int fingerIndex = 0; fingerIndex < fingerprints.Length; ++fingerIndex)
            {
                Fingerprint fingerprint = fingerprints[fingerIndex];
                TemplateBuilder builder = Extractor.Extract(fingerprint.Image, Dpi);
                fingerprint.Decoded = new SerializedFormat().Export(builder);
                templates.Add(fingerprint.Decoded);
                fingerIdexes.Add(fingerIndex);
            }
            return templates;
        }
